...
\only<1>{\includemedia[addresource=Daten/Chirp.mp3, flashvars={source=Daten/Chirp.mp3&autoPlay=true}]
	{\hspace{8pt}\includegraphics[scale=0.2]{Bilder/Chirp1.png}}{APlayer.swf}}
\only<2>{\includegraphics[scale=0.2]{Bilder/Chirp2.png}}
\only<3>{\hspace{4pt}\includegraphics[scale=0.2]{Bilder/Peak1.png}}
...
\begin{column}{0.6\textwidth}
	...
	\item<1-> unterschiedliche rücktreibende Kräfte \only<5->{$\rightarrow$ Potenz des Weges}
		\begin{itemize}
		\item<2->[-] Federkraft \only<6->{$\rightarrow$ $s^1$}
		\item<3->[-] Gravitation \only<7->{$\rightarrow$ nahezu $s^0$}
		\item<4->[-] Magnetkraft \only<8->{$\rightarrow$ $s^{-2}$}
		\end{itemize}
	\end{itemize}
\end{column}
\begin{column}{0.4\textwidth}
	\only<2>{\includegraphics[scale=0.1]{Bilder/Feder.jpg}}
	\only<4>{\includegraphics[scale=0.1]{Bilder/Magnete.jpg}}
\end{column}
\end{columns}
\end{frame}
