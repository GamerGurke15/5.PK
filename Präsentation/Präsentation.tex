...
\begin{itemize}
\item<1-> Energie bei maximaler Entfernung: $E=\frac{c}{a+1}\cdot s^{a+1}$
\item<2-> Energie beim Zusammenstoß: $E=\frac{m}{2}v^2$
\item<3-> Schwingung zwischen dieses Energien
	\begin{itemize}
	\item<3->[$\rightarrow$] $m\cdot \ddot{s}+c\cdot s^a=0$
	\end{itemize}
\end{itemize}
\end{frame}
